...
var HttpProxy = function() {
    ...
    return {
         ...
         * @param transferWeight {Integer}  The size of the script (gziped) in bytes, or null if the script was not gziped.
         * @param ungzipedWeight {Integer}  The size of the script (ungziped) in bytes.
        ...
        start: function(port, modifyFn) {
            ...
            app.use(function (req, res, next) {
                        ...
                        if (isGziped()) {
                            ...
                            zlib.gunzip(buffer, function(err, dezipped) {
                                if (err) {
                                    ...
                                    // Ungzip succeed
                                    // Transform!
                                    var newBody = modifyFn(dezipped, buffer.length, dezipped.length, req.url, index);
                            ...
                            // Not gziped
                            // Transform!
                            var newBody = modifyFn(buffer, buffer.length, buffer.length, req.url, index);
                ...
                function sendResponse(statusCode, headers, body) {
                    // Override cache headers, we don't want no cache!
